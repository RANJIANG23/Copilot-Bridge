...
$codexTestParent = Join-Path $env:LOCALAPPDATA 'CopilotBridge-Phase28'
...
$passed = $previousVersion -like '1.2.2*' -and
    $candidateVersion -like '1.3.0*' -and
    $rollbackVersion -like '1.2.2*' -and
...
if ($passed -and (Test-Path -LiteralPath $isolatedCodex)) {
    ...
    if (-not $resolvedCodexHome.StartsWith(
        ...
        [IO.Path]::GetFileName($resolvedCodexHome) -notlike 'v1.3.0-*') {
