...
$codexTestParent = Join-Path $env:LOCALAPPDATA 'CopilotBridge-Phase19'
...
$passed = $previousVersion -like '1.1.2*' -and
    $candidateVersion -like '1.2.0*' -and
    $rollbackVersion -like '1.1.2*' -and
...
if ($passed -and (Test-Path -LiteralPath $isolatedCodex)) {
    ...
    if (-not $resolvedCodexHome.StartsWith(
        ...
        [IO.Path]::GetFileName($resolvedCodexHome) -notlike 'v1.2.0-*') {
