...
{
    internal static void WriteInfo(string eventName, string details, string? path = null)
    {
        try
        {
            path ??= Path.Combine(
            ...
            var line = $"{DateTimeOffset.Now:O} [{SingleLine(eventName)}] {SingleLine(details)}{Environment.NewLine}";
