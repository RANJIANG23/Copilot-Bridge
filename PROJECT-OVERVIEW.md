...
- v1.3.0 已发布：新增可配置的 Assist、Outsource、Review 独立轮次预算和本地“数据统计”Dashboard；GUI 与 MCP 统一咨询生命周期，统计读取保持零写入并使用预计算缓存。
- v1.3.1 已发布：新增遵循系统减少动画设置的原生微动效，以及默认开启、在新 Edge/CDP 连接前生效的全屏保护；现有会话继续复用。
