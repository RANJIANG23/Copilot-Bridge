...
> 状态：v1.3.1 已正式发布；v1.3.2 Phase 36 保持部分完成；v1.4.0 Phase 37–39 已通过，Phase 40 自动门禁通过、人工视觉门禁未完成
> 工作名称：Copilot Bridge
> 项目目录：本仓库根目录
...
flowchart LR
    ...
    CORE --> RUNNER["Collaboration Runner"]
    RUNNER --> TURN["Copilot Turn Executor"]
    TURN --> DRIVER["Copilot Page Driver"]
    DRIVER -->|"CDP + DOM"| TAB["专用后台 Copilot 标签页"]
    TAB --> WEB["Microsoft 365 Copilot"]
    CORE --> PERSIST["Consultation Persistence"]
    PERSIST --> LOCAL["设置 / 状态 / Markdown 工作区"]
    WEB --> TAB --> DRIVER --> TURN --> RUNNER --> CORE --> MCP --> C
...
生产代码按真实变化原因划分职责，不为每个类再创建接口层：

1. **Settings / Consultation State Store**：原子读写配置和咨询预算状态；内存中的更高已消费轮次不得被旧磁盘记录回退。
2. **Consultation Coordinator**：统一 GUI 与 MCP 的策略、租约、模式、陈旧状态和预算门禁，并返回稳定 outcome；门禁通过前不获取 Edge 页面。
3. **Collaboration Runner**：只编排 Assist、Outsource 与双 reviewer Review，不知道 GUI、MCP、Edge 会话或本地存储。
4. **Copilot Turn Executor**：把单次请求连接到 Page Driver；开发 probe 也复用这一边界。
5. **Edge Session Adapter / Copilot Page Driver**：分别负责会话获取，以及模型选择、编辑器写入、发送与回复提取。
6. **Consultation Persistence / Conversation Workspace**：在已发送边界后保存状态、设置和 Markdown；保存失败只产生警告，绝不能变成可安全重试。
7. **Usage Statistics**：把本地 Markdown 一次预计算为只读统计数据集；UI 的日期和倍率重绘复用缓存，不重复扫描正文。
8. **MCP Host / GUI Shell**：只保留输入、DTO/控件映射和各自的 Edge session cache；不再复制咨询状态机。
...
- Codex 仍是主执行者。
- 初始请求只包含一个明确问题。
- v1.3.0 默认预算为 2 个 Copilot turn，可由用户在 1–20 范围内设置；咨询创建后固定预算。
...
- v1.3.0 默认预算为 6 个 Copilot turn，可由用户在 1–20 范围内设置；第 3 个 turn 后 Codex先检查是否仍有实质进展。
...
- Codex按证据裁决，不按多数票决定。
- v1.3.0 默认预算为 1 个完整双 reviewer 审查轮次，可由用户在 1–20 范围内设置；每轮仍保持两路隔离和串行执行。
...
Microsoft Copilot\
  ...
  src\
    CopilotBridge\
      ...
      UI\
        MainWindow.xaml
        MainWindow.xaml.cs           # 窗口生命周期和共享 UI 编排
        MainWindow.Consultation.cs   # 咨询 UI 适配
        MainWindow.Statistics.cs     # 数据统计 UI 适配
        UiText.cs
      Core\
        ConsultationCoordinator.cs
        ConsultationPersistence.cs
        ConsultationState.cs
        CollaborationRunner.cs
        CopilotTurnExecutor.cs
        ConversationWorkspace.cs
        ConversationStorageV2.cs
        UsageStatistics.cs
      Browser\
        EdgeSessionAdapter.cs
        CopilotPageDriver.cs
        ProviderSelectors.cs
      Mcp\
        CopilotBridgeTools.cs         # 四个工具的薄适配层
        McpHost.cs
      Resources\
...
不把 `Core`、`Browser`、`Mcp` 或 `UI` 拆成独立项目，也不增加 `Domain`、`Contracts`、`Adapters`、`WorkerRuntime` 等项目层。单一生产项目内的目录与命名空间足以表达边界。
...
- v1.1.2 移除了单一的应用级 `conversationTurnLimit`；旧字段在首次读取时原子移除。v1.3.0 新增的是三种模式各自的咨询预算，不恢复全局上限。
...
v1.3.0 的首个范围是把 Assist、Outsource、Review 的轮次预算放入“协作默认设置”下的新板块“协作模式设定”。详细配置、咨询快照、运行时门禁和验收以 [v1.3.0-design.md](./v1.3.0-design.md) 为准。

- 默认预算分别为 Assist 2、Outsource 6、Review 1，均允许用户在 1–20 范围内设置。
- 预算是新咨询默认值，创建咨询时固定到本地咨询记录；之后修改默认值不改变已有咨询。
- Review 的一个预算轮次始终包含 `complexity` 与 `evidence` 两个隔离 reviewer，reviewer 数量不可配置。
- 达到预算时必须在发送前返回 `turn_budget_exhausted`，发送零条消息且不可自动重试。
- `copilot_bridge_status` 只读返回三种默认预算；工具数量、写工具输入 schema、Edge/CDP/DOM 和禁止不确定重发的边界保持不变。
...
## 30. v1.3.1 原生体验与全屏连接保护

v1.3.1 以 [v1.3.1-design.md](./v1.3.1-design.md) 为权威范围：为已有 WPF 状态增加克制、可关闭的一次性微动效，并在团队真实全屏游戏切出问题的基础上，为新 CDP 连接增加默认开启的全屏保护。
...
- 不改变 MCP 工具数量、存储、统计、DOM、发送语义或禁止不确定重发的边界。
- 不增加 WebView、React、Motion、第三方 UI 包、动画框架或新生产依赖。
- 正式发布时源码、Plugin、构建脚本、下载与安装说明统一为 `1.3.1`。
...
v1.4.0 以 [v1.4.0-design.md](./v1.4.0-design.md) 为权威范围。该设计逐项核对 GLM 5.2 历史建议和当前实现，不把旧草案描述成未实施事实，也不为追求“全量”突破既定安全与复杂度边界。

- 已完成的主题、卡片、组件状态、键盘排序、存储 v2、系统托盘、减少动画和调用方中性化不重复建设。
- 本版新增会话标签、用户提示模板、可信原对话入口和待发送文本恢复，形成一个小而完整的本地工作台纵切。
- 本版补充默认关闭且可逆的当前用户开机启动、可跳过/可重开的首次使用清单、Windows 高对比度和有限响应式布局。
- Adaptive 自动路由、每对话模型暗控、多 profile/Provider、专用第三方适配器、HTML/PDF 导出、全局直发热键和 WorkBuddy 运行时接入不进入本版。
- 源码先进入 `1.4.0-dev`；1.3.2 候选构建、Plugin、安装说明和隔离升级证据保持不变。
- v1.4.0 生产 C# + XAML 总增量不超过 1600 行，不增加生产依赖、MCP 工具、服务、端口、数据库、RPC 或浏览器栈。
- Phase 36 的自动门禁证据继续有效，但人工 GUI 观察因无人工接管且外部 Windows 安全提示阻止安全操作而保持未完成；不倒签、不伪报通过。
