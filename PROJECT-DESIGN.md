...
> 状态：v1.4.0 已正式发布；Phase 0–39 与 G1–G8 已通过；Phase 40 自动门禁与正式发布完成、人工视觉门禁未完成，状态保持部分完成
> 工作名称：Copilot Bridge
> 项目目录：本仓库根目录
...
- 源码先进入 `1.4.0-dev`；已发布的 1.3.2 构建、Plugin、安装说明和隔离升级证据保持不变。
- v1.4.0 生产 C# + XAML 总增量不超过 1600 行，不增加生产依赖、MCP 工具、服务、端口、数据库、RPC 或浏览器栈。
- Phase 36 的正式发布证据继续有效；Phase 40 的人工 GUI 观察因无人工接管而保持未完成，不伪报该门禁通过。
