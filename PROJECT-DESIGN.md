...
复杂需求统一进入 [v1.2.0-design.md](./v1.2.0-design.md)，但进入该文档不等于具体实现方案已批准。v1.2.0 必须在 1.1.2 发布后先完成用户决策与预算门禁，再依次评估会话存储 v2、UI 设计系统与可访问性、托盘生命周期、调用方兼容和协作增强。

以下决定继续保留给用户：双标题同步、会话级模型控制、项目级受控调用和内容交接。Adaptive 不得恢复固定轮次上限；WorkBuddy 仍只允许作为开发期草稿工具，不进入产品运行时或发布物。
