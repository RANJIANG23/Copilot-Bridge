...
> 当前状态：Phase 0–5 已通过；Phase 6 RC3 发布候选版完成，G8 核心闭环通过、完整保护检查待验证
...
| Phase 6 | 部分完成 | 2026-07-18 05:42 +08:00 | 2026-07-18 06:14 +08:00（RC） | 32 分钟 | `phase-6-rc` | `win-x64` self-contained RC、稳定安装目录、幂等安装/卸载、开始菜单、团队 marketplace、Plugin、MCP、Skill 与三份部署文档已完成。本机全新卸载、安装、幂等重装和缓存后 MCP 启动通过；G7 的 Edge 重启、远程调试关闭、标签页关闭、登录失效和模型回退均产生预期结果。RC2 将“最近咨询”移至一级“历史对话”页。RC3 修复当前 Codex CLI 两行 marketplace 输出兼容性，并将升级改为事务式回退；新增两个隔离分发测试，Release 43/43 通过。本机真实完成 RC2→RC3 升级、RC3 卸载与恢复安装，应用、Plugin、marketplace、开始菜单及用户数据恢复正常。RC3 ZIP SHA-256：`b9ce35f6041b7f9b7c52d2532fbe4f21921a9a6d7b508522aef55897156b02e3`。G8 第二台电脑已完成安装、Assist 发送和回复读取；命令行参数拉起 Edge 会停留于 `Starting`，桌面正常启动的默认配置档工作正常，因此后者定为团队 v1 唯一支持路径。完整 G8 仍待配置保护、前台无抢占和卸载恢复检查。 |
