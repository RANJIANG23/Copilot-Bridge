...
> 当前状态：v1.3.2 已正式发布；Phase 34–39 已通过；v1.4.0 Phase 40 自动门禁通过、人工视觉门禁未完成
...
| Phase 38 | 本地工作台纵切 | 5–8 小时 | 标签、提示模板、可信恢复动作 | 通过 |
| Phase 39 | Windows 桌面就绪 | 4–7 小时 | 开机启动、首次使用、高对比度与有限响应式 | 通过 |
| Phase 40 | v1.4.0 候选门禁 | 4–7 小时 | 完整回归、预算、隔离升级与打包 | 部分完成 |
...
| Phase 38 | 通过 | 2026-07-27 03:40 +08:00 | 2026-07-27 03:46 +08:00 | 6 分钟 | `phase 38` | 会话标签经过顺序稳定、去空白、大小写去重和 12×32 字符上限规范化，只保存在 v2 sidecar，旧 sidecar 缺失字段继续读为空；工作台可编辑，授权元数据搜索可命中但不扩大项目权限。新增独立本地提示模板存储，支持保存、套用、重命名、删除、重名校验与临时文件替换，模板只填入原输入框。恢复动作可复制待发送文本，并只对精确允许 host 的 `/chat/conversation/` URL 打开原对话，不自动发送或重试。Debug 构建 0 警告/0 错误，专项 14/14、全量 196/196 测试通过；8633 行 C# + XAML，相对 Phase 37 增加 331 行（预算 900），2 个项目、2 个直接生产依赖、4 个 MCP 工具、`.workbuddy/` 跟踪文件 0。未连接 Edge、未发送消息、未修改用户工作区，未打包、安装、标记、推送或发布。 |
| Phase 39 | 通过 | 2026-07-27 03:47 +08:00 | 2026-07-27 03:55 +08:00 | 8 分钟 | `phase 39` | 新增默认关闭的当前用户 Startup 快捷方式，严格验证绝对目录，重复开关幂等，失败时恢复原快捷方式和设置，只启动同一 GUI EXE。首次使用清单可跳过、完成、在设置重开并导航到既有设置页，不自动配置 Edge、登录或发送。主题运行时监听 Windows 高对比度并映射系统窗口/文本/边框/高亮颜色，用户浅色/深色偏好不被改写；单一 1180 阈值把侧栏收敛到 220 px、工作台列收敛到 160/190 px 并隐藏次要侧栏说明。Debug 构建 0 警告/0 错误，桌面专项 40/40、全量 203/203 测试通过；8875 行 C# + XAML，阶段增量 242（预算 700）、v1.4.0 总增量 573（预算 1600），2 个项目、2 个直接生产依赖、4 个 MCP 工具、`.workbuddy/` 跟踪文件 0。测试只在临时目录创建/删除快捷方式；未改动真实 Startup、Edge、Copilot、用户工作区或正式安装，未打包、标记、推送或发布。 |
| Phase 40 | 部分完成 | 2026-07-27 03:56 +08:00 |  |  | `1b309a0` | Debug/Release 均为 0 警告/0 错误、203/203 测试通过；8875 行 C# + XAML，v1.4.0 总增量 573/1600，2 个项目、2 个直接生产依赖、4 个 MCP 工具、`.workbuddy/` 跟踪文件 0。候选包包含 611 个清单文件和 619 个归档条目，清单 611/611 匹配、根应用单一 `CopilotBridge.exe`、`.workbuddy/` 为 0，程序与 Plugin 均为 1.4.0，ZIP SHA-256 为 `ea0d428cedbd24837804e5b28c98c1ff079ca8aacb19fdbc0b03fd209fa953ff`。隔离环境完成 1.3.1→1.4.0、四工具通用 STDIO、状态和只读检索、用户数据与宿主配置保留、卸载和 1.3.1 回退；app-only 路径同样通过并完整卸载，证据位于 `artifacts/upgrade-test/v1.4.0-20260727040536`。打包 GUI 在隔离 `LOCALAPPDATA`（`artifacts/gui-smoke/v1.4.0-20260727040626`）下隐藏启动，进程保持响应后按 PID 停止；未连接 Edge、未发送消息、未改动真实 Startup、当前安装或用户工作区。人工中英文、高对比度、首次清单、工作台和最小窗口视觉检查因用户明确无人接管并要求跳过人工接入而未执行，因此不标记通过、不创建 Phase 40 通过提交。未安装、标记、推送或发布。 |
