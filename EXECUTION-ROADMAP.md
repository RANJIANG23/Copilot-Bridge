...
> 当前状态：v1.3.1 已正式发布；v1.3.2 Phase 36 保持部分完成；v1.4.0 Phase 37 已通过，Phase 38 进行中
...
| Phase 26 | 系统托盘生命周期 | 3–5 小时 | 关闭隐藏、恢复、显式退出与 MCP 语义 | 通过 |
| Phase 27 | v1.2.2 发布候选门禁 | 3–5 小时 | 迁移回滚、托盘观察、隔离升级与打包 | 通过 |
| Phase 28 | v1.3.0 协作模式预算 | 3–5 小时 | 三模式预算、咨询快照、发送前门禁、数据统计与架构收口 | 通过 |
| Phase 29 | v1.3.1 范围与开发基线冻结 | 1–2 小时 | 动效边界、减少动画、预算与版本一致 | 通过 |
| Phase 30 | 原生微动效纵切 | 3–5 小时 | 按钮、通知、开关与拖拽的系统动画门禁 | 通过 |
...
| Phase 37 | v1.4.0 GLM 对比与范围冻结 | 1–2 小时 | 事实校正、实施/拒绝边界与开发基线一致 | 通过 |
| Phase 38 | 本地工作台纵切 | 5–8 小时 | 标签、提示模板、可信恢复动作 | 进行中 |
| Phase 39 | Windows 桌面就绪 | 4–7 小时 | 开机启动、首次使用、高对比度与有限响应式 | 未开始 |
| Phase 40 | v1.4.0 候选门禁 | 4–7 小时 | 完整回归、预算、隔离升级与打包 | 未开始 |
...
| Phase 26 | 通过 | 2026-07-20 18:32 +08:00 | 2026-07-20 18:38 +08:00 | 6 分钟 | `phase 26` | 设置页新增默认关闭且可持久化的系统托盘选项；开关点击即保存并立即显示/移除图标，升级读取缺少该字段的旧设置仍保持关闭。开启后 NotifyIcon 在同一 GUI 进程内可见，普通窗口关闭只隐藏并保留窗口状态，双击或“打开 Copilot Bridge”恢复，显式“退出 Copilot Bridge”才进入既有 MCP 登记进程处理。Windows SessionEnding 绕过隐藏策略；未增加开机启动、服务、端口、进程扫描或 NuGet 依赖。Debug 构建 0 警告/0 错误，115/115 测试通过；6650 LOC、1 个生产项目 + 1 个测试项目、2 个直接依赖、`.workbuddy/` 跟踪文件 0。 |
| Phase 27 | 通过 | 2026-07-21 12:18 +08:00 | 2026-07-21 13:30 +08:00 | 1 小时 12 分钟 | `phase 27` | 修复首次提交前失败误复用不存在 consultationId 的契约，新增 `new_consultation` / `reuse_consultation` / `none`；模型选择复用已打开菜单，以有界 actionability 预检区分已知页面浮层和未知遮挡，并保持零强制点击、零自动关弹窗、零不确定重发。Debug/Release 均 0 警告/0 错误，125/125 测试通过；模型/浮层专项连续三轮 24/24，存储迁移/回滚与托盘策略专项 10/10。6775 物理 LOC（相对 5861 基线增量 914，预算 1100）、1 个生产项目 + 1 个测试项目、2 个直接依赖、`.workbuddy/` 跟踪文件 0。候选 ZIP 的 610 个清单文件全部匹配、618 个归档条目、单一生产 EXE、Plugin/程序 1.2.2，SHA-256 为 `34da46056b96a4a9778733c07fcf94c2e20178d7fd4085554720d4d9b843742a`。隔离环境完成 1.2.1→1.2.2、四工具 MCP、数据/宿主配置保留、卸载和 1.2.1 回退，证据位于 `artifacts/upgrade-test/v1.2.2-20260721122752`；候选 GUI 启动响应正常，用户人工确认托盘开关、关闭隐藏、托盘恢复与显式退出通过，并明确要求发布。 |
| Phase 28 | 通过 | 2026-07-21 15:45 +08:00 | 2026-07-22 01:49 +08:00 | 10 小时 4 分钟 | `phase 28` | 三模式预算默认 2/6/1、1–20 校验、咨询预算快照、GUI/MCP 发送前门禁、状态字段和双语 UI 已完成；审计后补齐 CRLF composer 等价校验、发送后失败状态留存、单调预算账本和统计完成率口径。GUI/MCP 已统一到 Coordinator，MCP 成为薄适配层；Dashboard 使用零写读取与预计算缓存。Debug/Release 均为 0 警告/0 错误，172/172 测试通过；7476 物理 LOC、1 个生产项目 + 1 个测试项目、2 个直接依赖、四个 MCP 工具、`.workbuddy/` 跟踪文件 0。候选包含 610 个清单文件和 618 个归档条目，程序与 Plugin 均为 1.3.0；隔离环境完成 1.2.2→1.3.0、四工具 MCP、数据/宿主配置保留、卸载和 1.2.2 回退，证据位于 `artifacts/upgrade-test/v1.3.0-20260722014814`；候选 GUI 启动响应正常，用户明确要求发布。最终 ZIP 哈希以 GitHub Release 同名 `.sha256` 文件为准。 |
| Phase 29 | 通过 | 2026-07-22 16:30 +08:00 | 2026-07-22 16:42 +08:00 | 12 分钟 | `phase 29` | v1.3.1 冻结为原生 WPF 微动效与减少动画正确性；固定按钮、通知、开关和现有拖拽四个范围、90–190 ms 时长、Phase 30–31 和 260 行 C# + XAML 增量预算。源码进入 `1.3.1-dev`，发布脚本、Plugin、下载和安装说明保持 1.3.0。Debug/Release 均 0 警告/0 错误，172/172 测试通过；基线为 7476 行 C#、7948 行 C# + XAML，1 个生产项目 + 1 个测试项目、2 个直接依赖、4 个 MCP 工具、`.workbuddy/` 跟踪文件 0。未打包、安装、标记、推送或发布。 |
| Phase 30 | 通过 | 2026-07-22 17:47 +08:00 | 2026-07-22 17:58 +08:00 | 11 分钟 | `phase 30` | 单一 `MainWindow.Motion.cs` 完成按钮 hover/pressed、通知首次淡入位移、开关 thumb 平移和既有拖拽的统一原生 WPF 动画；系统动画关闭时直接呈现最终状态，按钮保留原有即时模板反馈。新增测试覆盖系统门禁、90–190 ms 时长、开关非布局平移和允许渲染属性。Debug/Release 均 0 警告/0 错误，176/176 测试通过；8156 行 C# + XAML，相对基线增加 208 行，1 个生产项目 + 1 个测试项目、2 个直接依赖、4 个 MCP 工具、`.workbuddy/` 跟踪文件 0。未启动 GUI 视觉验收、未发送 Copilot 消息、未修改用户工作区，未打包、安装、标记、推送或发布。 |
...
| Phase 37 | 通过 | 2026-07-27 03:30 +08:00 | 2026-07-27 03:39 +08:00 | 9 分钟 | `phase 37` | 逐项校正 GLM 5.2 建议：已完成能力不重复建设；冻结会话标签、用户提示模板、可信恢复动作、可逆开机启动、首次使用、高对比度和有限响应式为 v1.4.0 范围；拒绝 Adaptive 暗路由、每对话模型暗控、多 profile/Provider、全局直发和 WorkBuddy 运行时接入。源码进入 `1.4.0-dev`，1.3.2 候选发布元数据与证据保持不变。Debug 构建 0 警告/0 错误，192/192 测试通过；8302 行 C# + XAML、2 个项目、2 个直接生产依赖、4 个 MCP 工具、`.workbuddy/` 跟踪文件 0。未连接 Edge、未发送消息、未修改用户工作区，未打包、安装、标记、推送或发布。 |
...
- 修复 v1.2.1 暴露的安全重试契约缺陷：首次提交前失败与既有咨询提交前失败必须分别返回 `new_consultation` / `reuse_consultation`，不确定提交继续返回 `none` 并禁止重试。
- 加固模型选择前置状态：复用已打开菜单、actionability 预检、未知页面浮层稳定错误码、Review 模式前置门禁，并以 fixture 证明全程零发送；禁止强制点击和自动关闭未知弹窗。
...
### 23.6 v1.2.2 之后的待做候选：原生 WPF 微动效

- 该项不进入 v1.2.2，也不重新打开 Phase 25–27；待 v1.2.2 完成后再由用户决定是否纳入 v1.2.3 或后续版本。
- 以 Amicro（React + Motion）作为交互参考，不直接复制网页运行时、不引入 WebView、React、Motion 或新的生产 NuGet 依赖；只用 WPF `Storyboard`、`DoubleAnimation`、`Opacity` 和 `RenderTransform` 重写。
- 首轮最多覆盖三类：按钮 hover/pressed 反馈、通知/操作结果的单次淡入位移、设置与托盘开关的滑块状态过渡。
- 不采用 magnetic 鼠标追踪、无限 glare/pulse、shake、focus blur、3D 卡片或其他持续/高干扰效果。
- 动效不改变布局、命中区域、键盘焦点或业务状态；关闭 Windows 动画时直接呈现最终状态，不能把动效作为唯一状态信号。
- 若复制或实质改写 Amicro 代码，发布物必须保留其 MIT 版权与许可声明；仅借鉴行为时仍在设计记录中保留来源。
- 进入实现前先冻结时长、缓动、适用控件和减少动画策略，并单独执行浅色/深色、键盘路径、最小窗口和实机视觉门禁。

## 24. Phase 28：v1.3.0 协作模式预算

### 24.1 目标

在不改变三种模式分工、MCP 工具数量和浏览器发送边界的前提下，让用户在“协作默认设置”内分别设置 Assist、Outsource、Review 的新咨询轮次预算。

### 24.2 交付物

- 新增“协作模式设定”板块与中文/English 文案。
- Assist、Outsource、Review 默认预算为 2、6、1，允许范围为 1–20。
- 设置原子持久化；新咨询创建时把实际预算快照到咨询状态。
- GUI 与 MCP 在发送前执行预算门禁；耗尽时返回 `turn_budget_exhausted`，零发送、不可自动重试。
- `copilot_bridge_status` 增加三个只读预算字段。
- 开发阶段源码版本进入 `1.3.0-dev`；正式发布时程序、构建脚本与 Plugin 统一为 1.3.0。
- GUI 与 MCP 共用一个咨询 Coordinator；所有发送前门禁在惰性获取 Edge page 之前完成。
- 数据统计原型使用零写工作区读取和预计算数据集，日期与倍率重绘不重复扫描正文。

### 24.3 明确不做

- 不恢复全局沟通轮次上限。
- 不改变 Review 固定双 reviewer、串行执行和角色隔离。
- 不增加工具、服务、端口、队列、数据库或浏览器自动化方案。
- 开发阶段不自行打包、安装、标记、推送或发布；只有用户明确授权后才进入正式发布流程。

### 24.4 验收

- 默认值、范围、持久化、旧设置兼容和咨询快照测试通过。
- 三种模式耗尽预算时执行器调用次数为零；最后一个合法轮次可以完成。
- Review 仍产生两个隔离结果，且完整双审计为一个预算轮次。
- 两个状态 Store 交错写、旧字符串状态和磁盘失败内存账本均不得导致 `turnCount` 回退。
- Dashboard 只读打开零写入，显式刷新之外的日期、语言和倍率变化只重绘缓存。
- `dotnet build CopilotBridge.sln` 与 `dotnet test CopilotBridge.sln --no-build` 通过。
- 更新阶段状态并创建本地提交：`phase 28: add configurable collaboration budgets`。

## 25. Phase 29–33：v1.3.1 原生体验与全屏连接保护

### 25.1 版本目标

为已有 WPF 状态增加克制的一次性微动效，并针对团队真实全屏游戏切出问题，在新 CDP 连接前增加默认开启的保护。存储、DOM 操作、发送状态机、禁止不确定重发和四工具边界保持不变。详细范围以 [v1.3.1-design.md](./v1.3.1-design.md) 为准。

### 25.2 Phase 29：范围与开发基线冻结

- 固定按钮、通知、开关和现有拖拽四个范围，以及 90–190 ms、原生 WPF、系统动画关闭直达最终状态的边界。
- 初始固定 Phase 30–31、260 行生产增量上限、2 个直接依赖和 4 个 MCP 工具。
- 正式发布时源码、Plugin、发布脚本、下载说明和发布状态统一为 `1.3.1`。
- 标准构建、既有测试、项目/依赖预算和 `.workbuddy/` 排除通过后创建本地提交。
- 本地提交：`phase 29: freeze v1.3.1 native motion scope`。

### 25.3 Phase 30：原生微动效纵切

- 使用 `SystemParameters.ClientAreaAnimation` 统一控制新增动画和现有拖拽反馈。
- 按钮增加 90–140 ms hover/pressed 反馈；通知以 180 ms 淡入位移显示；开关 thumb 以 160 ms 平移。
- 关闭 Windows 动画时立即设置最终状态，不延迟业务、不改变焦点和命中区域。
- 自动化测试、Debug/Release 和完整测试通过；不发送 Copilot 消息、不修改用户工作区。
- 本地提交：`phase 30: add reduced-motion-aware microinteractions`。

### 25.4 Phase 31：全屏保护范围与基线冻结

- 将真实团队案例限定为新 CDP 连接可能触发 Edge Remote access 提示，不把静态风险误报为每次事件的已证实根因。
- 固定默认开启的设置、只读窗口/显示器几何检测、`fullscreen_guard_active`、显式 GUI 例外和无标题/正文日志。
- 既有有效会话继续复用；拦截发生在 `ConnectOverCDPAsync` 和发送之前。
- 总 C# + XAML 增量预算修订为 420 行，直接依赖和 MCP 工具保持 2/4。
- 本地提交：`phase 31: freeze fullscreen connection protection`。

### 25.5 Phase 32：全屏连接保护纵切

- 使用只读 Win32 API 判断前台窗口是否覆盖所在显示器，不读取窗口标题或按进程名判断。
- GUI 自动刷新和 MCP 冷连接触发保护时不调用 `ConnectOverCDPAsync`，返回稳定提交前错误并保持安全重试契约。
- GUI 显式刷新、绑定和用户主动咨询可连接；设置页提供默认开启的保护选项。
- 自动化测试覆盖默认设置、持久化、几何、显式例外、错误映射、会话复用和零连接调用。
- Debug/Release、完整测试和复杂度边界通过；不发送 Copilot 消息、不修改用户工作区。
- 本地提交：`phase 32: guard new CDP connections during fullscreen use`。

### 25.6 Phase 33：视觉与发布候选门禁

- 新启动候选 GUI，由用户分别观察浅色/深色、动画开/关、全屏保护设置、键盘路径和最小窗口。
- 在非游戏状态显式建立授权会话，验证全屏下既有会话复用以及冷 MCP 连接被保护拦截。
- 完成完整构建测试、复杂度预算、候选包清单和隔离升级/回退。
- 候选不等于发布；正式打包、安装、标签、推送和 GitHub Release 等待用户明确授权。
- 本地提交：`phase 33: harden v1.3.1 release candidate`。

进度记录（2026-07-26）：

- 用户确认候选 GUI 的双主题、动画开/关、全屏保护设置、键盘路径和最小窗口显示正常。
- 用户确认全屏使用与退出全屏后的恢复表现正常。
- 真实原生全屏窗口冷连接门禁在 CDP 连接前返回 `fullscreen_guard_active`，未连接、未发送。
- Debug/Release 均为 186/186 测试通过、0 警告、0 错误；复杂度和前台操作边界扫描通过。
- 候选包包含 610 个清单文件和 618 个归档条目，清单 610/610 匹配、单一 EXE、`.workbuddy/` 为 0，程序与 Plugin 均为 1.3.1。
- 隔离环境完成 1.3.0→1.3.1 升级、四工具 MCP、只读检索零写入、用户数据与宿主配置保留、卸载和 1.3.0 回退。
- 用户已明确授权正式发布，Phase 33 标记为通过并创建阶段提交。
...
2026-07-27 补充：用户明确要求无人接管，并要求跳过必须人工接入的部分后继续。候选 GUI 已启动，但被与 Bridge 无关的 Windows 安全提示遮挡；依据安全操作边界未点击、关闭或绕过该提示。因此 Phase 36 继续保持“部分完成”，其 192/192 测试、候选包、通用 STDIO 和隔离升级证据不受影响；未创建补签的 Phase 36 通过提交。

### 26.5 延期边界

WorkBuddy、OpenClaw、opencode 等专用适配器，多 Agent 调度/身份/并发，自动内容交接，Adaptive 路由，Provider 框架和新的 MCP 工具不进入 v1.3.2。

## 27. Phase 37–40：v1.4.0 工作台与桌面就绪

### 27.1 版本目标

对 GLM 5.2 历史建议逐项进行当前源码事实校正，实施仍有价值且符合边界的工作台和 Windows 桌面能力。详细范围、预算与拒绝项以 [v1.4.0-design.md](./v1.4.0-design.md) 为准。

### 27.2 Phase 37：GLM 对比与范围冻结

- 把建议分类为已实施、本版实施、延期或拒绝，记录事实证据与冲突边界。
- 源码进入 `1.4.0-dev`；1.3.2 候选发布元数据和证据保持不变。
- 既有构建测试、项目/依赖/EXE/MCP 工具预算和 `.workbuddy/` 排除全部通过。
- 本地提交：`phase 37: freeze v1.4.0 workbench scope`。

### 27.3 Phase 38：本地工作台纵切

- 为会话存储 v2 增加兼容标签，工作台可编辑，授权内搜索可命中。
- 增加本地提示模板；模板只填入既有输入框，绝不自动发送。
- 增加可信原对话 URL 和待发送文本的显式恢复动作。
- 生产增量不超过 900 行，不增加依赖、工具、服务、端口或浏览器栈。
- 本地提交：`phase 38: add reusable local workbench tools`。

### 27.4 Phase 39：Windows 桌面就绪

- 增加默认关闭、当前用户、只启动 GUI 的可逆开机启动。
- 增加可跳过、可重开的首次使用清单，不自动登录、配置或发送。
- 遵从 Windows 高对比度，并在现有最小窗口内提供有限响应式侧栏与内容密度。
- Phase 39 增量不超过 700 行，v1.4.0 总增量不超过 1600 行。
- 本地提交：`phase 39: complete Windows desktop readiness`。

### 27.5 Phase 40：候选门禁

- 完成 Debug/Release、完整测试、增量预算、包清单和隔离升级/卸载/回退。
- GUI 人工门禁无法安全自动化时记录为未完成，继续其他门禁但不伪报 Phase 40 通过。
- 正式安装、标签、推送和 GitHub Release 等待用户明确授权。
- 本地提交：`phase 40: harden v1.4.0 candidate`（仅全部规定门禁通过时）。

### 27.6 延期和拒绝边界

Adaptive/关键词路由、每对话模型控制、自动标题同步、人可读文件名迁移、多 profile/Provider、专用第三方适配器、HTML/PDF 导出、全局直发热键、WorkBuddy 运行时接入和新的 MCP 工具不进入 v1.4.0。
