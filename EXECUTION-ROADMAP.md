...
> 当前状态：v1.3.0 已发布；v1.3.1 Phase 29–30 已通过，Phase 31 待开始
...
| Phase 31 | v1.3.1 视觉与发布候选门禁 | 3–5 小时 | 双主题、减少动画、隔离升级与打包 | 未开始 |
...
## 25. Phase 29–31：v1.3.1 原生 WPF 微动效

### 25.1 版本目标

在不改变 v1.3.0 业务和安全边界的前提下，为已有 WPF 状态增加克制的一次性微动效，并让现有拖拽反馈遵守 Windows 减少动画设置。详细范围以 [v1.3.1-design.md](./v1.3.1-design.md) 为准。

### 25.2 Phase 29：范围与开发基线冻结

- 固定按钮、通知、开关和现有拖拽四个范围，以及 90–190 ms、原生 WPF、系统动画关闭直达最终状态的边界。
- 固定 Phase 30–31、260 行生产增量上限、2 个直接依赖和 4 个 MCP 工具。
...
### 25.4 Phase 31：视觉与发布候选门禁

- 新启动候选 GUI，由用户分别观察浅色/深色、动画开/关、键盘路径和最小窗口。
- 完成完整构建测试、复杂度预算、候选包清单和隔离升级/回退。
- 候选不等于发布；正式打包、安装、标签、推送和 GitHub Release 等待用户明确授权。
- 本地提交：`phase 31: harden v1.3.1 release candidate`。

### 25.5 延期边界

页面转场、窗口启动、托盘图标动画、持续循环效果、完整 Adaptive、MVVM 重构、新 MCP 工具和任何业务增量不进入 v1.3.1。
