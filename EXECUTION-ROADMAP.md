...
> 当前状态：v1.4.0 已正式发布；Phase 0–39 与 G1–G8 已通过；Phase 40 自动门禁与正式发布完成、人工视觉门禁未完成，状态保持部分完成
...
| Phase 40 | 部分完成 | 2026-07-27 03:56 +08:00 |  |  | `v1.4.0` | 合并提交 `4ddbd84` 的 Debug/Release 均为 0 警告/0 错误、203/203 测试通过；8875 行 C# + XAML，v1.4.0 总增量 573/1600，2 个项目、2 个直接生产依赖、4 个 MCP 工具、`.workbuddy/` 跟踪文件 0。最终包包含 611 个清单文件和 619 个归档条目，清单 611/611 匹配、单一 `CopilotBridge.exe`、`.workbuddy/` 为 0，程序与 Plugin 均为 1.4.0，ZIP SHA-256 为 `117caa606315005ba00e48beb518202d9b67838d47cd175940eff8dc5d33e601`，可执行文件未签名。最终隔离证据 `artifacts/upgrade-test/v1.4.0-final-20260730012756` 完成 1.3.2→1.4.0、Plugin 与 app-only 四工具 STDIO、状态和只读检索、用户数据与宿主配置保留、卸载及 1.3.2 回退。PR #10 合并后，标签 `v1.4.0` 指向 `4ddbd84`，GitHub Release 的 ZIP 与 `.sha256` 已重新下载并三方哈希一致；本机已升级为 `1.4.0+4ddbd84`，Plugin、开始菜单和 MCP EOF 退出均核验。人工中英文、高对比度、首次清单、工作台和最小窗口视觉检查仍未执行，因此 Phase 40 不标记通过。 |
...
- 用户已于 2026-07-30 明确授权并完成正式安装、标签、推送和 GitHub Release。
