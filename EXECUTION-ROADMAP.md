...
> 当前状态：Phase 0–5 已通过；Phase 6 RC4 发布候选版完成，G8 核心闭环通过、完整保护检查待验证
...
| Phase 6 | 部分完成 | 2026-07-18 05:42 +08:00 | 2026-07-18 06:14 +08:00（RC） | 32 分钟 | `phase-6-rc` | `win-x64` self-contained RC、稳定安装目录、幂等安装/卸载、开始菜单、团队 marketplace、Plugin、MCP、Skill 与三份部署文档已完成。本机全新卸载、安装、幂等重装和缓存后 MCP 启动通过；G7 的 Edge 重启、远程调试关闭、标签页关闭、登录失效和模型回退均产生预期结果。RC2 将“最近咨询”移至一级“历史对话”页。RC3 完成升级事务式回退。RC4 改用 Codex CLI 官方 JSON 查询，删除两份自定义文本输出解析器，并补齐卸载事务边界：marketplace 查询失败时零删除，marketplace 移除失败时恢复原 Plugin；4 项隔离分发测试及 Release 45/45 测试通过。本机真实完成 RC4 卸载与恢复安装，应用、快捷方式、Plugin 和 marketplace 均正确删除并恢复，设置、历史及其他 Plugin/marketplace 全程不变。RC4 ZIP SHA-256：`d821448d0cc075ae06356b1c90b153a16766ae179c563dbe667378a8e7e6b734`。G8 第二台电脑已完成安装、Assist 发送和回复读取；命令行参数拉起 Edge 会停留于 `Starting`，桌面正常启动的默认配置档工作正常，因此后者定为团队 v1 唯一支持路径。完整 G8 仍待该电脑上的配置保护、前台无抢占和卸载恢复检查。 |
