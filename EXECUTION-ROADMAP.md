...
> 当前状态：Phase 0–6 与修订后的 G1–G8 已通过；v1.1 Phase 7–10 已通过；v1.1.2 Phase 11–13 已通过，Phase 14 进行中
...
| Phase 14 | v1.1.2 静默刷新与发布加固 | 2–4 小时 | 无周期闪烁、低风险入口、字体分发与发布候选门禁 | 进行中 |
...
| Phase 14 | 进行中 | 2026-07-20 02:25 +08:00 | — | — | — | Phase 13 已通过并准备形成本地阶段提交；开始概览静默刷新、即时咨询入口调整、打开工作区入口、Noto Sans SC 发布资源和最终发布候选验收。 |
...
存储格式、托盘生命周期、多调用方、Adaptive、内容交接和完整 UI 设计系统已从 v1.1.2 分离，统一记录在 [v1.2.0-design.md](./v1.2.0-design.md)。它们不得在 Phase 13 或 Phase 14 中提前实现。

只有 1.1.2 发布并且用户完成 v1.2.0 范围确认后，才能为 v1.2.0 建立新的执行 Phase 与状态表。
