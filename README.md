...
- `CopilotBridge-1.4.0-win-x64.zip`
- `CopilotBridge-1.4.0-win-x64.zip.sha256`
...
```powershell
(Get-FileHash .\CopilotBridge-1.4.0-win-x64.zip -Algorithm SHA256).Hash.ToLowerInvariant()
```
...
| 项目 / Item | 状态 / Status |
|---|---|
| 当前源码版本 / Current source version | `1.4.0`（工作台与桌面就绪） / `1.4.0` (workbench and desktop readiness) |
| 发布状态 / Release status | v1.4.0 已发布 Windows x64 自包含安装包与 SHA-256 文件 / v1.4.0 released with a Windows x64 self-contained package and SHA-256 file |
| 已通过 / Passed | Phase 0–39 and G1–G8；Phase 40 自动门禁与正式发布已完成，人工 GUI 门禁保持未完成 / Phase 0–39 and G1–G8; Phase 40 automated gates and formal release are complete while its human GUI gate remains incomplete |
...
团队 v1.4.0 已完成项目定义的自动化与本机隔离门禁，但不把本机验收描述为跨设备兼容性证明，也不把尚未执行的人工视觉检查表述为已通过。`1.4.0` 已作为 Windows x64 自包含安装包发布；安装前请核对 GitHub Release 中的同名 `.sha256` 文件。

Team v1.4.0 completed the project's automated and local isolated gates, but local acceptance is not presented as proof of cross-device compatibility, and the pending manual visual checks are not reported as passed. `1.4.0` is released as a Windows x64 self-contained package; verify the matching `.sha256` file in the GitHub Release before installation.
